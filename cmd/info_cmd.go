...
var InfoCommand = cli.Command{
 ...
 You can use the ./edge info block --help command to view help information.`,
}
...
func blockInfo(ctx *cli.Context) error {
	...
	if len(arg) > 30 {
		...
		data, err = cutils.GetBlock(height)
		if err != nil {
			return fmt.Errorf("GetBlock error:%s", err)
		}
	}
...
func txInfo(ctx *cli.Context) error {
	...
	txInfo, err := cutils.GetRawTransaction(ctx.Args().First())
	if err != nil {
...
func txStatus(ctx *cli.Context) error {
	...
	evtInfos, err := cutils.GetSmartContractEventInfo(txHash)
	if err != nil {
...
func curBlockHeight(ctx *cli.Context) error {
	count, err := cutils.GetBlockCount()
	if err != nil {
		return err
	}
...
func showTx(ctx *cli.Context) error {
	...
	txHash := tx.Hash()
	height, _ := cutils.GetTxHeight(txHash.ToHexString())
